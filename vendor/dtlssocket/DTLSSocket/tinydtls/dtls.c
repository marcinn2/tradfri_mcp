...
static const dtls_user_parameters_t default_user_parameters = {
  ...
  .force_extended_master_secret = 0,
  .force_renegotiation_info = 0,
};
...
dtls_send_client_key_exchange(dtls_context_t *ctx, dtls_peer_t *peer)
{
  ...
  fprintf(stderr, "DEBUG dtls_send_client_key_exchange kex=%d\n", key_exchange_algorithm); fflush(stderr);
...
check_server_hello(dtls_context_t *ctx,
		      ...
		      uint8 *data, size_t data_length)
{
  fprintf(stderr, "DEBUG check_server_hello data_length=%zu\n", data_length); fflush(stderr);
  dtls_handshake_parameters_t *handshake = peer->handshake_params;
  ...
  {
    int ext_res = dtls_check_tls_extension(peer, data, data_length, 0);
    fprintf(stderr, "DEBUG dtls_check_tls_extension returned %d data_length=%zu\n", ext_res, data_length); fflush(stderr);
    return ext_res;
  }
}
...
{
  int res;
  fprintf(stderr, "DEBUG check_server_hellodone called state=%d\n", peer->state); fflush(stderr);
#ifdef DTLS_ECC
  const dtls_ecdsa_key_t *ecdsa_key;
#endif /* DTLS_ECC */
...
handle_handshake_msg(dtls_context_t *ctx, dtls_peer_t *peer, uint8 *data, size_t data_length) {
  ...
  fprintf(stderr, "DEBUG handle_handshake_msg type=%d state=%d\n", ((dtls_handshake_header_t*)data)->msg_type, state); fflush(stderr);
  const cipher_suite_key_exchange_algorithm_t key_exchange_algorithm =
  ...
  case DTLS_HT_HELLO_VERIFY_REQUEST:
    ...
    /* Reset sequence dedup so server's next flight (starting at seq 0) isn't dropped */
...
dtls_handle_message(dtls_context_t *ctx,
  ...
  while ((rlen = is_record(msg,msglen))) {
    ...
    fprintf(stderr, "DEBUG dtls_handle_message looking up peer\n"); fflush(stderr);
    peer = dtls_get_peer(ctx, session);
    if (peer) {
        fprintf(stderr, "DEBUG PEER FOUND\n"); fflush(stderr);
        dtls_debug("dtls_handle_message: FOUND PEER\n");
    } else {
      fprintf(stderr, "DEBUG PEER NOT FOUND\n"); fflush(stderr);
      if (data) {
