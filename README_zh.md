...
- **CoAP OBSERVE 推播通知** -- 遙控器或 Apple Home 操作燈時，透過 Telegram 通知你，讓你被自己的房子監控
...
> **安全聲明：** 本專案設計用於受信任的家庭內網環境。MCP Server 不包含認證或 TLS 加密機制。請勿在未加裝額外安全措施的情況下將服務埠暴露到公網。
...
## CoAP OBSERVE（又名「你家會打小報告」）

Server 啟動時會對每個 alias 建立 CoAP OBSERVE 訂閱。當燈的狀態改變——比方說有人按了實體遙控器或用 Apple Home 操作——server 偵測到變化後就會透過 Telegram 發送通知。沒錯，你的另一半開個廁所燈你都會收到推播。後果自負。

**前置需求：** 在 `.env` 設定 `TELEGRAM_BOT_TOKEN` 和 `TELEGRAM_CHAT_ID`。

**行為：**
- 啟動時擷取 baseline 狀態
- 只在**狀態變更**時通知（謝天謝地，不會通知初始值）
- OBSERVE 訂閱中斷時自動重連（重試間隔：`TRADFRI_POLL_INTERVAL`，預設 30 秒）
- 不干擾控制操作（OBSERVE 擁有 CoAP context 生命週期，我們用慘痛的教訓學會了為什麼這很重要）
...
在 Telegram 對 OpenClaw 說「開客廳燈」，然後沐浴在成功的光芒中（字面意義上的）。
...
原始設計中 `coap_put` / `coap_get` 失敗時會 `_ctx = None` 重置 context。聽起來很合理對吧？但這會破壞正在運作的 OBSERVE session，因為 TRADFRI gateway 每個 PSK identity 只允許一個 DTLS session。哎呀。
...
曾經嘗試用 `asyncio.Semaphore(1)` + 2 秒延遲來序列化 OBSERVE 初始化，認為 20 個並發 GET 會壓垮 gateway。驕兵必敗——實測證明 gateway 可以處理並發 OBSERVE GET，問題根源是上面的 context reset bug，不是並發量。

移除 semaphore 後 OBSERVE 初始化時間從 ~40 秒降到幾秒。有時候最好的最佳化就是刪掉自己寫的「聰明」程式碼。
